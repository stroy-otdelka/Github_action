...
def run_workflow(token: str, name: str):
    ...
    data = {"ref": "develop/fix_github_actions"}
...
def get_workflow_info(token: str, name: str):
    headers = {
        "Accept": "application/vnd.github+json",
        "Authorization": f"Bearer {token}",
        "X-GitHub-Api-Version": "2022-11-28",
...
def wait_workflow(token: str, name: str):
    run_workflow(token, name)
    print(f"workflow {name} выполняется...")
    conclusion = None
    while not conclusion:
        time.sleep(5)
        conclusion = get_workflow_info(token, name)
    return conclusion
...
if __name__ == '__main__':
    ...
    conclusion_res = wait_workflow(args.token, args.workflow_name)
